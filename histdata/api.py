...
class URL:
    ...
    NINJA_TRADER_LAST_QUOTES = (
        "https://www.histdata.com/download-free-forex-historical-data/?/ninjatrader/tick-last-quotes/"
    )
    NINJA_TRADER_BID_QUOTES = (
        "https://www.histdata.com/download-free-forex-historical-data/?/ninjatrader/tick-bid-quotes/"
    )
    NINJA_TRADER_ASK_QUOTES = (
        "https://www.histdata.com/download-free-forex-historical-data/?/ninjatrader/tick-ask-quotes/"
    )
...
def download_hist_data(
    ...
    if (not tick_data) and (
        (int(year) >= datetime.now().year and month is None)
        or (int(year) <= datetime.now().year - 1 and month is not None)
    ):
    ...
    r = requests.post(url="https://www.histdata.com/get.php", data=data, headers=headers)
    ...
    with zipfile.ZipFile(zip_file, "r") as z:
        # Extract the first file in the zip (assuming only one file)
    ...
    if month is None:
        output_filename = "DAT_{}_{}_{}_{}.csv".format(platform, pair.upper(), time_frame, str(year))
    else:
        output_filename = "DAT_{}_{}_{}_{}.csv".format(
            platform, pair.upper(), time_frame, "{}{}".format(year, str(month).zfill(2))
        )
    ...
    if verbose:
        print("Saved extracted data to:", output_filename)
